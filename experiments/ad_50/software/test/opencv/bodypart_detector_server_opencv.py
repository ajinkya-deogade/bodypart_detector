...
bodypart_vote = []
bodypart_knn_pos = cv2.ml.KNearest_create()
bodypart_knn_neg = cv2.ml.KNearest_create()
...
def main(options, args):
    ...
    surf = cv2.xfeatures2d.SURF_create(bodypart_trained_data_pos.hessianThreshold, bodypart_trained_data_pos.nOctaves, bodypart_trained_data_pos.nOctaveLayers, 1)
    ...
    bodypart_knn_pos.train(np.array(bodypart_trained_data_pos.descriptors,dtype=np.float32), cv2.ml.ROW_SAMPLE, bodypart_trained_data_pos.keypoints)
    bodypart_knn_neg.train(np.array(bodypart_trained_data_neg.descriptors,dtype=np.float32), cv2.ml.ROW_SAMPLE, bodypart_trained_data_neg.keypoints)
    ...
    while 1:
        ...
        for i in range(0, packet["list"]["length"]):
            ...
            if options.opencv_keypoints != 1:
                ...
                # print "Computed SURF Descriptor...."
            else:
                kp_frame, desc_frame = surf.detectAndCompute(frame, None)
                # print "Computed SURF Descriptor...."

            # print "Descriptor Length: ", len(desc_frame)
            # print "Keypoint Length: ", len(kp_frame)

            for h, desc in enumerate(desc_frame):
                desc = np.array(desc, np.float32).reshape((1, 128))
                retval_pos, results_pos, neigh_resp_pos, dists_pos = bodypart_knn_pos.findNearest(desc, 1)
                retval_neg, results_neg, neigh_resp_neg, dists_neg = bodypart_knn_neg.findNearest(desc, 1)
