...
def writeFPGATrainingFile(trainDescriptors, trainVotes, train_bodypart, trainFileName):
    ...
    # print 'Data All Shape : ', len(dat_all)
    # # print dat_all[0]
...
def train(train_annotation_list, project_dir, train_bodypart, hessianThreshold, nOctaves, nOctaveLayers, pos_neg_equal, mh_neighborhood, crop_size, dir_keypoints, dir_descriptors):
    ...
    bodypart_kp_train_true_neg = []
    bodypart_desc_train_true_neg = []

    training_bodypart = train_bodypart
    keypoints_all = 0
    fileNotFound = 0
    framesDOMissing = 0
    totalPositiveBodyparts = 0
    ...
    for i in range(0, len(train_annotation["Annotations"])):
        ...
        bodypart_coords_positive = []
        bodypart_coords_negative = []
        bodypart_coords_gt = []
        ...
        bodypart_kp_train_true_neg_frame = []
        bodypart_desc_train_true_neg_frame = []

        crop_center_x = -1
        crop_center_y = -1

        for j in range(0, len(train_annotation["Annotations"][i]["FrameValueCoordinates"])):
            bodyPartName = train_annotation["Annotations"][i]["FrameValueCoordinates"][j]["Name"]
            bodyPartLoc_x = train_annotation["Annotations"][i]["FrameValueCoordinates"][j]["Value"]["x_coordinate"]
            bodyPartLoc_y = train_annotation["Annotations"][i]["FrameValueCoordinates"][j]["Value"]["y_coordinate"]

            if bodyPartLoc_x != -1 and bodyPartLoc_y != -1:
                bodypart_coords_positive.append({"x" : int(bodyPartLoc_x), "y" : int(bodyPartLoc_y), "bodypart" : bodyPartName})

            if bodyPartName == "LeftDorsalOrgan":
            # if bodyPartName == "RightDorsalOrgan":
                if bodyPartLoc_x == -1 and bodyPartLoc_y == -1:
                    bodypart_coords_negative.append({"x" : int(bodyPartLoc_x), "y" : int(bodyPartLoc_y), "bodypart" : bodyPartName})

            if (bodyPartName == "MouthHook" and bodyPartLoc_x != -1 and bodyPartLoc_y != -1):
                crop_center_x = int(bodyPartLoc_x)
                crop_center_y = int(bodyPartLoc_y)
            elif (bodyPartName == "LeftMHhook" and bodyPartLoc_x != -1 and bodyPartLoc_y != -1):
                crop_center_x = int(bodyPartLoc_x)
                crop_center_y = int(bodyPartLoc_y)
            elif (bodyPartName == "RightMHhook" and bodyPartLoc_x != -1 and bodyPartLoc_y != -1):
                crop_center_x = int(bodyPartLoc_x)
                crop_center_y = int(bodyPartLoc_y)
            elif (bodyPartName == "LeftDorsalOrgan" and bodyPartLoc_x != -1 and bodyPartLoc_y != -1):
                crop_center_x = int(bodyPartLoc_x)
                crop_center_y = int(bodyPartLoc_y)
            elif (bodyPartName == "RightDorsalOrgan" and bodyPartLoc_x != -1 and bodyPartLoc_y != -1):
                crop_center_x = int(bodyPartLoc_x)
                crop_center_y = int(bodyPartLoc_y)

        framesDOMissing += len(bodypart_coords_negative)
        totalPositiveBodyparts += len(bodypart_coords_positive)

        if (bodypart_coords_positive != [] and crop_center_x != -1 and crop_center_y != -1):
            ...
            if not os.path.exists(keypoints_folder):
                print "Keypoint Folder does not exist !!!"
            ...
            if not os.path.exists(descriptors_folder):
                print "Descriptor Folder does not exist !!!"
            ...
            for k in range(0, len(bodypart_kp)):
                ...
                for bodypart_id in range(0, len(bodypart_coords_positive)):
                    if (np.sqrt(np.square(x - bodypart_coords_positive[bodypart_id]["x"]) +
                                np.square(y - bodypart_coords_positive[bodypart_id]["y"])) <= mh_neighborhood):
                        flag_add_to_pos = True
                        bodypart_dp_pos = np.array([bodypart_coords_positive[bodypart_id]["x"] - x, bodypart_coords_positive[bodypart_id]["y"] - y]).T
                        bodypart_R_pos = np.array([[np.cos(a), -np.sin(a)], [np.sin(a), np.cos(a)]]).T
                        bodypart_dp_R_pos = np.dot(bodypart_R_pos, bodypart_dp_pos)
                        votes_tmp_pos.append((bodypart_dp_R_pos, bodypart_coords_positive[bodypart_id]["bodypart"]))

                flag_add_to_true_neg = False
                for bodypart_id in range(0, len(bodypart_coords_negative)):
                    # if bodypart_coords_negative[bodypart_id]["bodypart"] == 'LeftDorsalOrgan' or bodypart_coords_negative[bodypart_id]["bodypart"] == 'RightDorsalOrgan':
                    flag_add_to_true_neg = True

                if (flag_add_to_pos):
                    bodypart_kp_train_pos_frame.append(bodypart_kp[k])
                    bodypart_desc_train_pos_frame.append(bodypart_desc[k])
                    bodypart_vote_train_pos_frame.append(votes_tmp_pos)

                if (flag_add_to_true_neg):
                    bodypart_kp_train_true_neg_frame.append(bodypart_kp[k])
                    bodypart_desc_train_true_neg_frame.append(bodypart_desc[k])

                flag_add_to_neg = False
                for bodypart_id in range(0, len(bodypart_coords_positive)):
                    if (np.sqrt(np.square(x - bodypart_coords_positive[bodypart_id]["x"]) +
                                np.square(y - bodypart_coords_positive[bodypart_id]["y"])) >= 1.5*mh_neighborhood):
            ...
            if pos_neg_equal:
                ...
                bodypart_kp_train_true_neg.extend(bodypart_kp_train_true_neg_frame)
                bodypart_desc_train_true_neg.extend(bodypart_desc_train_true_neg_frame)
    ...
    print "Frames Not Found : ", fileNotFound
    print "Frames Without DO : ", framesDOMissing
    print "Total Positive Bodyparts : ", totalPositiveBodyparts
    ...
    print "Number of True Negative Training Samples: ", len(bodypart_kp_train_true_neg)
    ...
    bodypart_desc_train_samples_true_neg = np.array(bodypart_desc_train_true_neg)
    ...
    timestr = time.strftime("%Y%m%d_%H%M%S")
    trainFileName = '../expts/' + timeStampFolder + '/' + timestr + '_trainingParameters.json'
    results = open(trainFileName, 'w')

    trainList = '../expts/' + timeStampFolder + '/' + timestr + '_trainList.lst'
    ...
    trainData["NumberTrueNegativeKeypoint"] = len(bodypart_kp_train_true_neg)
    ...
    trainPosFileName = os.path.join(timeStampFolder_FGPA, timestr + '_' + training_bodypart[0] + '_trainPositiveData.txt')
    trainNegFileName = os.path.join(timeStampFolder_FGPA, timestr + '_' + training_bodypart[0] + '_trainNegativeData.txt')
    trainTrueNegFileName = os.path.join(timeStampFolder_FGPA, timestr + '_' + training_bodypart[0] + '_trainTrueNegativeData.txt')

    writeFPGATrainingFile(bodypart_desc_train_samples_pos, bodypart_vote_train_pos, train_bodypart, trainPosFileName)
    writeFPGATrainingFile(bodypart_desc_train_samples_neg, [], train_bodypart, trainNegFileName)
    writeFPGATrainingFile(bodypart_desc_train_samples_true_neg, [], train_bodypart, trainTrueNegFileName)
...
def detect(detect_bodypart, project_dir, train_data_p, train_data_n, test_annotation_list, vote_sigma, vote_patch_size, desc_distance_threshold, vote_threshold, outlier_error_dist):
    ...
    test_annotations = []
    with open(test_annotation_list) as tl:
        for test_annotation_file in tl:
...
if __name__ == '__main__':
    ...
    train(train_annotation_list, project_dir, train_bodypart, hessianThreshold, nOctaves, nOctaveLayers, pos_neg_equal, mh_neighborhood, crop_size, keypt_dir, desc_dir)