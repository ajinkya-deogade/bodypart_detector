...
def main(options, args):
    ...
    frame_index = -1
    error_stats_all = []
    meanErrorInterpFile = string.join(['../expts/error_interpolation_',test_bodypart.strip(),".txt"])
    annotatedPositionFile = string.join(['../expts/original_positions_',test_bodypart.strip(),".txt"])
    interpolatedPositionFile = string.join(['../expts/interpolated_positions_',test_bodypart.strip(),".txt"])
    distanceVectorFile = string.join(['../expts/distance_vector_',test_bodypart.strip(),".txt"])
    fileWriter_1 = open(meanErrorInterpFile,'w+')
    fileWriter_2 = open(annotatedPositionFile,'w+')
    fileWriter_3 = open(interpolatedPositionFile,'w+')
    fileWriter_4 = open(distanceVectorFile,'w+')

    for i in range(0, len(test_annotations)):
        bodypart_gt = {}
        bodypart_coords_gt = {}
        bodypart_coords_interpolated = {}
        bodypart_coords_est = {}
        ...
        if(cap.isOpened()):
            ...
            for k in range(0, len(annotation)):
                ...
                n += 1
                ret, frame = cap.read()
                bodypart_coords_gt[k] = {}
                bodypart_coords_interpolated[k] = {}
                ...
                for j in range(0, len(annotation[k]["FrameValueCoordinates"])):
                    if (annotation[k]["FrameValueCoordinates"][j]["Name"] == test_bodypart):
                        bodypart_coords_gt[k]["x"] = int(annotation[k]["FrameValueCoordinates"][j]["Value"]["x_coordinate"])
                        bodypart_coords_gt[k]["y"] = int(annotation[k]["FrameValueCoordinates"][j]["Value"]["y_coordinate"])
                        bodypart_coords_gt[k]["FrameIndexVideo"] = int(annotation[k]["FrameIndexVideo"])

                if ( bodypart_coords_gt[k] == None ):
                    print "Empty !!!"
                    continue
                ...
                bodypart_gt[frame_index]["bodypart_coords_gt"] = bodypart_coords_gt[k]
                # bodypart_gt[frame_index]["bodypart_coords_interpolated"] = bodypart_coords_interpolated[k]
                bodypart_gt[frame_index]["video_file"] = video_file
                bodypart_gt[frame_index]["frame_number"] = n
                ...
                print "bodypart_coords_gt:" , bodypart_coords_gt[k]
                # bodypart_coords_interpolated[k] = bodypart_coords_gt[k]
                # print "bodypart_coords_interpolated:" , bodypart_coords_interpolated[k]
                crop_x = 0
                crop_y = 0
                ...
                for s in range(0, n_server):
                    if ( not server_free[s] ):
                            ...
                            if ("detections" in received_json):
                                for di in range(0, len(received_json["detections"])):
                                    if ( received_json["detections"][di]["test_bodypart"] == test_bodypart ):
                                        bodypart_coords_est[k] = {}
                                        bodypart_coords_est[k]["x"] = received_json["detections"][di]["coord_x"]
                                        bodypart_coords_est[k]["y"] = received_json["detections"][di]["coord_y"]
                        ...
                        #     error_stats.video_file = bodypart_gt[f_index]["video_file"]
                        #     error_stats.frame_number = bodypart_gt[f_index]["frame_number"]
                        ...
                        #     error_stats.error_distance_interpolation_est = np.sqrt(np.square(bodypart_gt[f_index]["bodypart_coords_interpolated"]["x"] - bodypart_coords_est["x"]) +
                        #                                                            np.square(bodypart_gt[f_index]["bodypart_coords_interpolated"]["y"] - bodypart_coords_est["y"]))
                        #     error_stats.error_distance_interpolation_annot = np.sqrt(np.square(bodypart_gt[f_index]["bodypart_coords_interpolated"]["x"] - bodypart_coords_gt["x"]) +
                        #                                                              np.square(bodypart_gt[f_index]["bodypart_coords_interpolated"]["y"] - bodypart_coords_gt["y"]))
                        ...
                        #     print bodypart_gt[f_index]["video_file"],bodypart_gt[f_index]["frame_number"], "Distance between interpolated and annotated RightMHhook location:", error_stats.error_distance_interpolation_annot
                        #     print bodypart_gt[f_index]["video_file"],bodypart_gt[f_index]["frame_number"], "Distance between interpolated and estimated RightMHhook location:", error_stats.error_distance_interpolation_est
        ...
        distance_coord_gt_interp[k] =[]
        for m in range(0, len(bodypart_coords_gt)):
            # print "BodyCoord Interpolated: ", bodypart_coords_gt[m]
            if (m > 0 and m < ((len(bodypart_coords_gt)-1))):
                if ((bodypart_coords_gt[m-1]["x"] != -1) and (bodypart_coords_gt[m+1]["x"] != -1)):
                    bodypart_coords_interpolated[m]["x"] = int((bodypart_coords_gt[m-1]["x"]+bodypart_coords_gt[m+1]["x"])/2)
                    bodypart_coords_interpolated[m]["y"] = int((bodypart_coords_gt[m-1]["y"]+bodypart_coords_gt[m+1]["y"])/2)
                    temp = np.sqrt(np.square(bodypart_coords_gt[m]["x"]-bodypart_coords_interpolated[m]["x"]) + np.square(bodypart_coords_gt[m]["y"]- bodypart_coords_interpolated[m]["y"]))
                    distance_coord_gt_interp[k].append(temp)
                    distance_coord_gt_interp_tot.append(temp)
                else:
                    bodypart_coords_interpolated[m]["x"] = int(bodypart_coords_gt[m]["x"])
                    bodypart_coords_interpolated[m]["y"] = int(bodypart_coords_gt[m]["y"])
            else:
                bodypart_coords_interpolated[m]["x"] = int(bodypart_coords_gt[m]["x"])
                bodypart_coords_interpolated[m]["y"] = int(bodypart_coords_gt[m]["y"])
        ...
        # strPrint = string.join(['Distance between interpolated and annotated position for file : ', i,'\t', distance_coord_gt_interp,'\n'])
