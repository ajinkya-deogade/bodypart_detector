...
def train(train_annotation_list, project_dir, train_bodypart, hessianThreshold, nOctaves, nOctaveLayers, pos_neg_equal, mh_neighborhood, crop_size, dir_keypoints, dir_descriptors):
    ...
    for i in range(0, len(train_annotation["Annotations"])):
        ...
        if (bodypart_coords != [] and bodypart_coords_gt != []):
            ...
            try:
                ...
                print 'len(bodypart_kp) ', len(bodypart_kp)
    ...
    timeStampFolder_fpga = timeStampFolder + '/fpga/'
...
def detect(detect_bodypart, project_dir, train_data_p, train_data_n, test_annotation_list, vote_sigma, vote_patch_size, desc_distance_threshold, vote_threshold, outlier_error_dist, timeStampFolder, dir_keypoints, dir_descriptors):
    ...
    for j in range(0, len(test_annotations)):
        ...
        if "MouthHook" in bodypart_gt[frame_index]["bodypart_coords_gt"]:
            ...
            bodypart_vote_map, numberKP = computeVoteMapOPENCV(bodypart_knn_pos, bodypart_knn_neg,
                                                               bodypart_trained_data_pos, bodypart_trained_data_neg,
                                                               frame, vote_patch_size, vote_sigma, bodypart_kp, bodypart_desc, detect_bodypart)
...
if __name__ == '__main__':
    ...
    train_flag = 1
    if train_flag > 0:
        train_data_positive, train_data_negative, timeStampFolder = train(train_annotation_list, project_dir, train_bodypart, hessianThreshold, nOctaves, nOctaveLayers, pos_neg_equal, mh_neighborhood, crop_size, keypt_dir, desc_dir)
    else:
        timeStampFolder = '../expts/all_7_except_Rawdata_20170317_232257/fpga/'
        train_data_positive = '../expts/all_7_except_Rawdata_20170317_232257/fpga/positive.p'
        train_data_negative = '../expts/all_7_except_Rawdata_20170317_232257/fpga/negative.p'

    detect_bodypart = train_bodypart

    testList = test_annotation_list_all
    Y = []
    with open(testList) as all_list:
        for test_annotation_list in all_list:
            Y.append(test_annotation_list)

    test_annotation_list = Y

    # detect(detect_bodypart, project_dir, train_data_positive, train_data_negative, test_annotation_list, vote_sigma, vote_patch_size, desc_distance_threshold, vote_threshold, outlier_error_dist, timeStampFolder, keypt_dir, desc_dir)