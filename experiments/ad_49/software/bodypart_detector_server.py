...
surf = cv2.xfeatures2d.SURF_create(int(500), nOctaves=int(2), nOctaveLayers=int(3), extended=1)
...
def main(options, args):
    ...
    bodypart_knn_pos = cv2.flann.Index(np.array(bodypart_trained_data_pos.descriptors, dtype=np.float32), dict(algorithm=1, trees=4))
    bodypart_knn_neg = cv2.flann.Index(np.array(bodypart_trained_data_neg.descriptors, dtype=np.float32), dict(algorithm=1, trees=4))

    bodypart_vote = np.zeros((2 * options.vote_patch_size + 1, 2 * options.vote_patch_size + 1), np.float)
    ...
    print "Server listening on port", options.socket_port
    sock.listen(1)
    ...
    while 1:
        ...
        for i in range(0, packet["list"]["length"]):
            ...
            ack_message["received"] += " " + str(image_header["rows"]) + "x" + str(image_header["cols"])
        ...
        if ( options.verbosity >= 1 ):
            ...
            print "len(ack_message):", len(ack_message)
