                ...
                if ( not server_free[s] ):
                    ...
                    if (bodypart_coords_est is not None):
                        ...
                        if (options.display_level >= 1):
                            ...
                            # display_vote_map = cv2.resize(display_vote_map, (0, 0), fx=0.75, fy=0.75)
                            cv2.imshow("voters", display_vote_map)
                            cv2.waitKey(100)
                            if options.save_dir:
                                crop_x = max(0, bodypart_coords_mh["x"]-100)
                                crop_y = max(0, bodypart_coords_mh["y"]-100)
                                display_vote_map_cropped = display_vote_map[crop_y:crop_y+512, crop_x:crop_x+512, :]
                                current_dir = os.path.abspath(os.path.dirname(frame_file))
                                parent_dir = os.path.basename(current_dir)
                                save_folder=os.path.join(options.save_dir, parent_dir)
                                if not os.path.exists(save_folder):
                                    os.makedirs(save_folder)
                                save_name = os.path.join(save_folder, os.path.splitext(os.path.basename(annotation["FrameFile"]))[0]) + ".jpeg"
                                cv2.imwrite(save_name, display_vote_map_cropped*255, (cv2.cv.CV_IMWRITE_JPEG_QUALITY,50))
                if ( options.verbosity >= 1 ):
                    print server_free
...
if __name__ == '__main__':
    ...
    parser.add_option("", "--save-dir", dest="save_dir",default="../expts/images", help="degree of verbosity")
