        ...
        if (bodypart_coords != []):
            ...
            for k in range(0, len(bodypart_kp)):
                ...
                for bodypart_id in range(0, len(bodypart_coords)):
                    ...
                    if (np.sqrt(np.square(x - bodypart_coords[bodypart_id]["x"]) + 
                                np.square(y - bodypart_coords[bodypart_id]["y"])) >= 2*options.mh_neighborhood):
            ...
            bodypart_kp_train_neg, bodypart_desc_train_neg = zip(*random.sample(zip(bodypart_kp_train,bodypart_desc_train),len(bodypart_kp_train)/20))
