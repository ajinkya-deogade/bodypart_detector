...
def detect_bodypart_and_get_error_distances(annotation):
    ...
    global bodypart_knn_pos, bodypart_knn_neg, bodypart_trained_data_pos, k
    ...
    kp_frame, desc_frame = surf_descriptorExtractor.compute(image_cropped, keypoints)
    ...
    for h, desc in enumerate(desc_frame):
        ...
        retval_pos, results_pos, neigh_resp_pos, dists_pos = bodypart_knn_pos.find_nearest(desc, k)
        retval_neg, results_neg, neigh_resp_neg, dists_neg = bodypart_knn_neg.find_nearest(desc, k)
    ...
    bodypart_coords_est = {}
    bodypart_coords_est["x"] = int(vote_max_loc[1]) + crop_x
    bodypart_coords_est["y"] = int(vote_max_loc[0]) + crop_y
    ...
    if (options.save_dir_images != ""):
        ...
        save_name = os.path.join(save_folder, os.path.splitext(os.path.basename(annotation["FrameFile"]))[0]) + ".jpeg"
