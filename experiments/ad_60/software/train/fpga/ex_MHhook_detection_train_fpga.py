...
<<<<<<< HEAD
            ...
            with open(csv_file, 'r') as csvfile:
                ...
                for row in csvreader:
                    ...
                    row = row[0]
                    keypoints_temp = cv2.KeyPoint(x=float(row[0]), y=float(row[1]), _size=float((float(row[2])/float(1.2))*float(9)), _angle=float(-1), _response=float(row[3]), _octave=int(row[4]), _class_id=int(row[5]))
                    keypoints.append(keypoints_temp)

            bodypart_kp, bodypart_desc = surf.detectAndCompute(frame, keypoints)
            print "Number of Keypoints Detected: ", len(bodypart_kp)
            for k in range(0, len(bodypart_kp)):
                x, y = bodypart_kp[k].pt
                print "Orientation: ", bodypart_kp[k].angle
                x = x + crop_x
                y = y + crop_y
