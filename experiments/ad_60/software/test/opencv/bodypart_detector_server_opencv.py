...
def main(options, args):
    ...
    surf = cv2.SURF(bodypart_trained_data_pos.hessianThreshold, bodypart_trained_data_pos.nOctaves, bodypart_trained_data_pos.nOctaveLayers)
    ...
    while 1:
        ...
        for i in range(0, packet["list"]["length"]):
            ...
            cv2.imshow("Image Received", frame)
            # cv2.waitKey(500)
            ...
            if ( vote_max > options.vote_threshold and vote_max > frame_vote_max ):
                ...
                if (options.display_level >= 1):
                    ...
                    if ( "x" in bodypart_coords_est ):
                        cv2.circle(display_vote_map, (bodypart_coords_est["x"], bodypart_coords_est["y"]), 4, (0, 0, 255), thickness=-1)
                        cv2.imshow("voters", display_vote_map)
                        # cv2.waitKey(500)
                        if (options.save_dir != ""):
                            print "Saved !!"
                            save_name = os.path.join(options.save_dir, "000" + str(packet["list"]["frame_index"])) + ".jpeg"
...
if __name__ == '__main__':
    ...
    parser.add_option("", "--save-dir", dest="save_dir", default="", help="degree of verbosity")
