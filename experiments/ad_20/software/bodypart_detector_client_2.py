...
def main(options, args):
    ...
    annotatedPositionFile = os.path.join('../expts/annotated_positions_' + test_bodypart.strip() + ".json")
    ...
    allMeansMediansFile = os.path.join('../expts/allMeans&Medians_' + test_bodypart.strip() + ".json")
    allTimeFile = os.path.join('../expts/allTime_' + test_bodypart.strip() + ".json")
    confidenceFile = os.path.join('../expts/confidence_' + test_bodypart.strip() + ".json")
    ...
    for videoNumber in range(0, len(test_annotations)):
        ...
        for m in range(0, len(bodypart_coords_gt[videoNumber]) - 1):
            if (m > 0 and m < ((len(bodypart_coords_gt[videoNumber]) - 2))):
                if ((bodypart_coords_gt[videoNumber][m - 1]["x"] != -1) and (
                            bodypart_coords_gt[videoNumber][m + 1]["x"] != -1) and (
                            bodypart_coords_gt[videoNumber][m]["x"] != -1)):
    ...
    all_Means_Medians["Confidence"]["MeanDistance"] = np.mean(confidence_votes)
    all_Means_Medians["Confidence"]["MedianDistance"] = np.median(confidence_votes)
