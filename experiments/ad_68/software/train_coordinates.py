        ...
        if (options.train_annotation_file != ""):
            ...
            with open(options.train_annotation_list) as fin_annotation_list:
                for train_annotation_file in fin_annotation_list:
                    ...
                    k = 0
                    data_posterior = np.hstack([train_annotation[:-time_lag], train_annotation_mh[:-time_lag], train_annotation_lmh[:-time_lag], train_annotation_rmh[:-time_lag]]) ## Exclude the last five elements
                    for i in range(time_lag-1, 0, -1):
                        k += 1
                        data_posterior = np.hstack([data_posterior, train_annotation[k:-i], train_annotation_mh[k:-i], train_annotation_lmh[k:-i], train_annotation_rmh[k:-i]])
                    ...
                    try:
                        ...
                        indWithAnnotation = np.nonzero(X_all!=-1.0)
                        X_diff = X_pred[indWithAnnotation] - X_all[indWithAnnotation]
                        Y_diff = Y_pred[indWithAnnotation] - Y_all[indWithAnnotation]
