...
unpacker_keypoint_data = struct.Struct('= 2s d')

unpacker_descriptor_header = struct.Struct('= 2s I I')
unpacker_list_descriptor_header = struct.Struct('= 2s I I')
unpacker_descriptor_data = struct.Struct('= 2s I')
...
def main(options, args):
    ...
    while 1:
        ...
        for i in range(0, packet["list"]["length"]):
            ...
            bodypart_vote_map = []
            for bid in range(0, len(options.detect_bodypart)):
            ...
            if  keypoint_header["rows"]* keypoint_header["cols"] != len(keypoint_buffer):
            ...
            gap = conn.recv(gap_size.size)
            if options.verbosity >0:
                print "Gap Descriptors: ", gap

            descriptors = []
            data = conn.recv(unpacker_descriptor_data.size)
            header = unpacker_descriptor_data.unpack(data)
            if options.display_level > 0:
                print "Keypoint Data Header:",header
            packet = {}
            packet["version"] = header[0]
            ...
            packet["size"] = header[1]
            if options.verbosity > 0:
                print "descriptor packet version:" , packet["version"]
                print "descriptor packet size:" , packet["size"]

            data = conn.recv(unpacker_list_descriptor_header.size)
            list_header = unpacker_list_descriptor_header.unpack(data)
            packet["list"] = {}
            packet["list"]["version"] = list_header[0]
            assert packet["list"]["version"] == "00"
            packet["list"]["frame_index"] = list_header[1]
            packet["list"]["length"] = list_header[2]
            if options.display_level > 0:
                print "descriptor list version:" , packet["list"]["version"]
                print "descriptor list frame_index:" , packet["list"]["frame_index"]
                print "descriptor list length:" , packet["list"]["length"]

            ack_message='{ "received" : "' + str(packet["list"]["frame_index"])
            frame_vote_max = -1
            bodypart_coords_est = {}

            data = conn.recv(unpacker_descriptor_header.size)
            header_data = unpacker_descriptor_header.unpack(data)
            descriptor_header = {}
            descriptor_header["version"] = header_data[0]
            assert descriptor_header["version"] == "01"
            descriptor_header["rows"] = header_data[1]
            descriptor_header["cols"] = header_data[2]

            if options.verbosity > 0:
                print "descriptor header version:", descriptor_header["version"]
                print "descriptor header num rows:", descriptor_header["rows"]
                print "descriptor header num cols:", descriptor_header["cols"]

            descriptor_buffer = conn.recv(packet["list"]["length"])
            descriptor_buffer = np.frombuffer(descriptor_buffer, dtype = np.float32)
            if  descriptor_header["rows"]* descriptor_header["cols"] != len(descriptor_buffer):
                print "Number of Rows and Columns Do Not Match"
                continue
            if options.verbosity > 0:
                print "descriptor buffer size:", descriptor_buffer.shape

            descriptors_data = descriptor_buffer.reshape((descriptor_header["rows"], descriptor_header["cols"]))
            ...
            for h, desc in enumerate(desc_frame):
                ...
                if (relative_distance <= options.desc_distance_threshold):
                    ...
                    for bi in range(0,len(bodypart_trained_data_pos.votes[r_pos])):
                        ...
                        x, y = p
                        if (not (x <= options.vote_patch_size or x >= np.shape(frame)[1] - options.vote_patch_size or
                                 y <= options.vote_patch_size or y >= np.shape(frame)[0] - options.vote_patch_size)):
                            bodypart_vote_map[vi][y - options.vote_patch_size:y + options.vote_patch_size + 1,
            ...
            for bi in range(0, len(options.detect_bodypart)):
                vote_max = np.amax(bodypart_vote_map[bi][:,:])
                if ( vote_max > options.vote_threshold and ((options.detect_bodypart[bi] not in bodypart_coords_est) or vote_max > bodypart_coords_est[options.detect_bodypart[bi]]["conf"]) ):
                    frame_vote_max = vote_max
                    vote_max_loc = np.array(np.where(bodypart_vote_map[bi][:,:] == vote_max))
                    vote_max_loc = vote_max_loc[:,0]
                    bodypart_coords_est[options.detect_bodypart[bi]] = {"conf" : vote_max,
                                                                        "x" : int(vote_max_loc[1]) + int(image_header["crop_x"]),
                                                                        "y" : int(vote_max_loc[0]) + int(image_header["crop_y"])}

        ack_message["detections"] = []
        for bi in options.detect_bodypart:
            if ( bi in bodypart_coords_est ):
                ack_message["detections"].append( { "frame_index" : packet["list"]["frame_index"],
                                                    "test_bodypart" : bi,
