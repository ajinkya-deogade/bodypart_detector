...
def detect(detect_bodypart, project_dir, test_annotation_list, vote_sigma, vote_patch_size, vote_threshold, train_data_p, train_data_n, outlier_error_dist):
    ...
    for bid in detections_error_all:
        ...
        print "Number of inlier error distances (beyond %d) = %d / %d = %g" % (options.outlier_error_dist, len(error_distance_inliers), n_annot_gt_total[bid], float(len(error_distance_inliers)) / max(1, float(n_annot_gt_total[bid])) )
