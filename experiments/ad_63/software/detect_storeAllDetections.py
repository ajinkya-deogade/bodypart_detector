...
def detect(detect_bodypart, project_dir, train_data_p, train_data_n, test_annotation_list, vote_sigma, vote_patch_size, desc_distance_threshold, vote_threshold, outlier_error_dist):
    ...
    for bid in error_stats_all:
        ...
        results.write("Body part = "+ bid)
        results.write("Number of inliers = "+ len(error_distance_inliers))
        results.write("Ground truth number of instances = "+ n_instance_gt[bid])
        results.write("Total number of detections = "+ len(error_stats_all[bid]))
        results.write("Outlier error distances = " +  float(n_outlier) / max(1, float(n_instance_gt[bid])))
        ...
        if (len(error_distance_inliers) > 0):
            results.write("Median inlier error dist = "+ np.median(error_distance_inliers))
            results.write("Mean inlier error dist = "+ np.mean(error_distance_inliers))
            results.write("Min inlier confidence = "+ np.min(inlier_confs))
            results.write("Mean inlier confidence = "+np.mean(inlier_confs))
        ...
        if (len(outlier_confs) > 0):
            results.write("Max outlier confidence = "+ np.max(outlier_confs))
            results.write("Mean outlier confidence = "+ np.mean(outlier_confs))
...
if __name__ == '__main__':
    ...
    parser.add_option("", "--pos-neg-equal", dest="pos_neg_equal", default=0, type="int", help="Input the bodypart to be trained")
    ...
    if train_flag > 0:
        ...
        train_data_positive = '/Volumes/Macintosh HD/Ajinkya/mhdo/experiments/ad_63/expts/20160125_230659_fragmented/positive.p'
        train_data_negative = '/Volumes/Macintosh HD/Ajinkya/mhdo/experiments/ad_63/expts/20160125_230659_fragmented/negative.p'
