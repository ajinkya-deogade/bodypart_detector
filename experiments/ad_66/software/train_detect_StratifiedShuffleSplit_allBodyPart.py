...
def train(train_annotation_list, project_dir, train_bodypart, hessianThreshold, nOctaves, nOctaveLayers, pos_neg_equal, mh_neighborhood, crop_size, dir_keypoints, dir_descriptors):
    ...
    for i in range(0, len(train_annotation["Annotations"])):
        ...
        if (bodypart_coords != [] and bodypart_coords_gt != []):
            ...
            # frame = frame[crop_y:crop_y+256, crop_x:crop_x+256, 0]
    ...
    for i, desc in enumerate(bodypart_desc_train_samples_pos):
        ...
        for vote in votes:
            if vote[1] in train_bodypart:
                ...
                bp_present[pos] = 1
                pos2 = 3*pos
                dat_local_desc[pos2] = round(vote[0][0], 4)
                dat_local_desc[pos2+1] = round(vote[0][1], 4)
...
if __name__ == '__main__':
    ...
    # train_annotation_list_all = '../config/temp'
    train_annotation_list_all = '../config/temp_2'
    ...
    if train_flag > 0:
        train_data_positive, train_data_negative = train(train_annotation_list, project_dir, train_bodypart, hessianThreshold, nOctaves, nOctaveLayers, pos_neg_equal, mh_neighborhood, crop_size, keypt_dir, desc_dir)
    else:
    ...
    # test_annotation_list_fpgaKNNVal = "../config/train_annotation_list_clips"
    #
    # detect(detect_bodypart, project_dir, train_data_positive, train_data_negative, test_annotation_list_fpgaKNNVal, vote_sigma, vote_patch_size, desc_distance_threshold, vote_threshold, outlier_error_dist)