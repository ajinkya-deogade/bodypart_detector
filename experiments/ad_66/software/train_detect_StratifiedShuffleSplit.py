...
def train(train_annotation_list, project_dir, train_bodypart, hessianThreshold, nOctaves, nOctaveLayers, pos_neg_equal, mh_neighborhood, crop_size):
    ...
    trainFileName = '../expts/' + timestr + '_trainData.dat'
    ...
    for i, desc in enumerate(bodypart_desc_train_samples_neg):
        dat = np.empty([1, 1+len(desc)+16], dtype=np.float64)
        np.put(dat, 0, sum(desc))
        np.put(dat, np.arange(1, 128, 128, dtype=int), desc)
        np.put(dat, [129, 132, 135, 138, 141], [0, 0, 0, 0, 0])
    ...
    print 'Before Deleting ', np.shape(dat_all)[0]
    # for k in range(0, np.shape(dat_all)[0]):
    #     if dat_all[k][129] == 0 and dat_all[k][132] == 0 and dat_all[k][135] == 0 and dat_all[k][138] == 0 and dat_all[k][141] == 0:
...
if __name__ == '__main__':
    ...
    # strat = np.hstack((0*np.ones((1, 5), dtype=np.int), 1*np.ones((1, 5), dtype=np.int), 2*np.ones((1, 5), dtype=np.int))) ## Dummy List annotation_list_old_new_all_forStratified_2
    # strat = np.hstack((0*np.ones((1, 2), dtype=np.int), 1*np.ones((1, 2), dtype=np.int))) ## Dummy List 2 annotation_list_old_new_all_forStratified_4
    # X = []
    #
    ...
    #
    # totalLen = len(X)
    # testDataLen = int(np.floor(0.50*totalLen))
    ...
    # train_annotation_list_all = '../config/annotation_list_old_new_all_forStratifiedForFPGA'
    #train_annotation_list_all = '../config/annotation_list_old_new_all_forStratified_4'

    train_annotation_list_all = '../config/20170227_testTraining'
    ...
    # test_annotation_list_fpgaKNNVal = X[X_test_idx[:,0]]
    #
