        ...
        if ( "detections" in received_json ):
            for di in range(0, len(received_json["detections"])):
                ...
                if ( tbp in bodypart_gt[fi]["bodypart_coords_gt"] ):
                    ...
                    error_stats.detection_x = received_json["detections"][di]["coord_x"]
                    error_stats.detection_y = received_json["detections"][di]["coord_y"]
                    if (verbosity >= 1 ):
...
if __name__ == '__main__':
    ...
    with open(train_annotation_list_all) as all_list:
        for train_annotation_list in all_list:
            trainList.append(train_annotation_list)

    trainList = np.array(trainList, dtype=np.str)
    # print trainList
    train_flag = 0

    if train_flag > 0:
        train_data_positive, train_data_negative = train(trainList, project_dir, train_bodypart, hessianThreshold, nOctaves, nOctaveLayers, pos_neg_equal, mh_neighborhood, crop_size)
    else:
        train_data_positive = "../expts/20160324_174029_fragmented/positive.p"
        train_data_negative = "../expts/20160324_174029_fragmented/negative.p"
    ...
    with open(test_annotation_list_all) as all_list:
        for test_annotation_list in all_list:
            testList.append(test_annotation_list)

    testList = np.array(testList, dtype=np.str)

    detect(detect_bodypart, project_dir, train_data_positive, train_data_negative, testList, vote_sigma, vote_patch_size, desc_distance_threshold, vote_threshold, outlier_error_dist)