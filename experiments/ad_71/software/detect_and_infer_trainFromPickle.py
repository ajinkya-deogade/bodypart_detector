...
def detect(detect_bodypart, project_dir, test_annotation_list, vote_sigma, vote_patch_size, vote_threshold, train_data_p):
    ...
    for j in range(0, len(test_annotations)):
        ...
        if "MouthHook" in bodypart_gt[frame_index]["bodypart_coords_gt"]:
            ...
            voteMapOPENCV = computeVoteMapOPENCV(bodypart_knn_pos, bodypart_trained_data_pos, frame, vote_patch_size, vote_sigma)
        else:
            ## Empty the buffer
            bufferPredictedPositions = {}
            for bi in detect_bodypart:
        ...
        if (display_level >= 3):
            for bi in detect_bodypart:
                ...
                if ("x" in bodypart_coords_est_o[bi]):
                    ...
                    if bi in bodypart_gt[frame_index]["bodypart_coords_gt"]:
                    ...
                    if bi in infer_bodypart:
                        if ("x" in bodypart_coords_inf_o[bi]):
                            ...
                            cv2.circle(displayFrame, (int(inference_OPENCV_X), int(inference_OPENCV_Y)), 4, (0, 255, 0), thickness=-1)
        ...
        if ("inference" in received_json_o):
            for di in received_json_o["inference"]:
                tbp = di["test_bodypart"]
                fi = di["frame_index"]
                if (tbp in bodypart_gt[fi]["bodypart_coords_gt"]):
                    error_stats = Error_Stats()
                    error_stats.frame_file = bodypart_gt[fi]["frame_file"]
                    error_stats.error_distance = np.sqrt(np.square(bodypart_gt[fi]["bodypart_coords_gt"][tbp]["x"] - di["coord_x"]) + np.square(bodypart_gt[fi]["bodypart_coords_gt"][tbp]["y"] - di["coord_y"]))
                    if (verbosity >= 1):
                        print "Frame Index:", frame_index, "\nDistance between annotated and inferred", tbp, "location:", error_stats.error_distance
                    error_stats_all_f[tbp].append(error_stats)

    print "\n.............Errors after OpenCV Detection......."
