...
def detect(detect_bodypart, project_dir, test_annotation_list, vote_sigma, vote_patch_size, vote_threshold, train_data_p, train_data_n, outlier_error_dist):
    ...
    for test_annotation_file in test_annotation_list:
        ...
        with open(test_annotation_file) as fin_annotation:
            ...
            for j in range(0, len(test_annotations)):
                ...
                vote_map = computeVoteMapOPENCV(bodypart_knn_pos, bodypart_knn_neg, bodypart_trained_data_pos, bodypart_trained_data_neg, frame, vote_patch_size, vote_sigma)
                ...
                for bi in ["LeftDorsalOrgan", "RightDorsalOrgan"]:
                    ...
                    if (vote_max > vote_threshold):
                        ...
                        if ('LeftMHhook' in bodypart_coords_est) & ('RightMHhook' in bodypart_coords_est) & ('MouthHook' in bodypart_coords_est):
                            tx = (bodypart_coords_est['LeftMHhook']['x']+bodypart_coords_est['RightMHhook']['x'])/2 - bodypart_coords_est['MouthHook']['x']
                            ty = (bodypart_coords_est['LeftMHhook']['y']+bodypart_coords_est['RightMHhook']['y'])/2 - bodypart_coords_est['MouthHook']['y']
                            vecH = np.array([tx, ty])
                            mh = np.array([bodypart_coords_est['MouthHook']['x'], bodypart_coords_est['MouthHook']['y']])
                            do = np.array([bodypart_coords_est[bi]['x'], bodypart_coords_est[bi]['y']])
                            mh_do = do - mh
                            crossDOH[bi] = np.cross(mh_do, vecH)

                        if bi == 'LeftDorsalOrgan' and crossDOH[bi] < 0:
                            addToDetection[bi] = 0

                        if bi == 'RightDorsalOrgan' and crossDOH[bi] > 0:
                            addToDetection[bi] = 0
                    else:
                        addToDetection[bi] = 0

                    if addToDetection[bi]:
                        # print "Was able to detect bodypart", bi
                        bodypart_coords_inf[bi] = {"x": int(vote_max_loc[1]) + int(image_header["crop_x"]), "y": int(vote_max_loc[0]) + int(image_header["crop_y"])}
                        buffer_detection[bi].append([int(vote_max_loc[1]) + int(image_header["crop_x"]), int(vote_max_loc[0]) + int(image_header["crop_y"])])
                ...
                if (display_level >= 3):
                    ...
                    # print "Cross LH ", crossLH
                    # if 'LeftDorsalOrgan' in crossDOH:
                    #     cv2.putText(inferredFrame, 'Cross Product LH (%d)'%(crossDOH['LeftDorsalOrgan']), (1100, 250), font, 2, (255,0,0), 8)
                    # if 'RightDorsalOrgan' in crossDOH:
                    #     cv2.putText(inferredFrame, 'Cross Product RH (%d)'%(crossDOH['RightDorsalOrgan']), (1100, 350), font, 2, (0,0,255), 8)
                    ...
                    if bodypart_coords_gt["LeftDorsalOrgan"]['x'] != -1 and bodypart_coords_gt["LeftDorsalOrgan"]['y'] != -1:
                    ...
                    if bodypart_coords_gt["RightDorsalOrgan"]['x'] != -1 and bodypart_coords_gt["RightDorsalOrgan"]['y'] != -1:
                    ...
                    if bp in bodypart_coords_gt:
                        if bp in bodypart_coords_est:
                            if bodypart_coords_gt[bp]["x"] != -1 and bodypart_coords_gt[bp]["y"] != -1:
                                detections_error[bp][j] = np.sqrt(np.power((int(bodypart_coords_est[bp]['x']) - int(bodypart_coords_gt[bp]['x'])), 2) + np.power((int(bodypart_coords_est[bp]['y']) - int(bodypart_coords_gt[bp]['y'])), 2))

                        if bp in bodypart_coords_inf:
                            if bodypart_coords_gt[bp]["x"] != -1 and bodypart_coords_gt[bp]["y"] != -1:
                                inferences_error[bp][j] = np.sqrt(np.power((int(bodypart_coords_inf[bp]['x']) - int(bodypart_coords_gt[bp]['x'])), 2) + np.power((int(bodypart_coords_inf[bp]['y']) - int(bodypart_coords_gt[bp]['y'])), 2))
    ...
    for bp in infer_bodypart:
        ...
        for a in range(0, len(bins)):
            print "Proportion of no correct detections for %d frames leading to correct inference %0.02f"%(a, float(noDetectionButInferenceAll[bp][a])/max(1,float(noDetectionAll[bp][a])))
    ...
    #     print "Percentage of missed inferences for bodypart %s (%d/%d) = %0.03f " %(bp, noInference_all[bp], continuousDetection_all[bp], float(noInference_all[bp])*100/max(float(1), float(continuousDetection_all[bp])))
