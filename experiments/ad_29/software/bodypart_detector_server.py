...
def main(options, args):
    ...
    data_pos =  []
    with open(options.train_positive_files_list) as fin_positive_training_list:
        for training_positive_file in fin_positive_training_list:
            training_positive_file = training_positive_file.strip()
            trained_data = SaveClass()
            trained_data = pickle.load(open(training_positive_file, 'rb'))
            data_pos.append(trained_data)

    votes = []
    descriptors = []
    keypoints = []
    for dat in data_pos:
        votes.extend(dat.votes)
        descriptors.extend(dat.descriptors)
        keypoints.extend(dat.keypoints)
        bodypart_pos = dat.bodypart

    votes = np.array(votes)
    descriptors = np.array(descriptors)
    keypoints = np.array(keypoints)
    combined_pos = zip(votes, descriptors, keypoints)
    combined_pos = np.array(combined_pos)

    print "Votes", np.shape(votes)
    print "Descriptors", np.shape(descriptors)
    print "Keypoints", np.shape(keypoints)
    print "Combined", np.shape(combined_pos)
    combined_pos = np.sort(combined_pos, axis = 0, kind='mergesort')
    print "Combined After", np.shape(combined_pos)
    votes, descriptors, keypoints = zip(*combined_pos)

    bodypart_trained_data_pos.votes = np.array(np.sort(votes))
    bodypart_trained_data_pos.descriptors = np.array(descriptors)
    bodypart_trained_data_pos.keypoints = np.array(keypoints)
    bodypart_trained_data_pos.bodypart = bodypart_pos

    bodypart_trained_data_neg = SaveClass()

    data_neg = []
    with open(options.train_negative_files_list) as fin_negative_training_list:
        for training_negative_file in fin_negative_training_list:
            training_negative_file = training_negative_file.strip()
            trained_data = SaveClass()
            trained_data = pickle.load(open(training_negative_file, 'rb'))
            data_neg.append(trained_data)
    ...
    for dat in data_neg:
        ...
        descriptors.extend(dat.descriptors)
        keypoints.extend(dat.keypoints)
        bodypart_neg = dat.bodypart

    votes = np.array(votes)
    descriptors = np.array(descriptors)
    keypoints = np.array(keypoints)
    combined_neg = zip(votes, descriptors, keypoints)
    combined_neg = np.sort(combined_neg, axis=0)
    print "Votes", np.shape(votes)
    print "Descriptors", np.shape(descriptors)
    print "Keypoints", np.shape(keypoints)
    print "Combined", np.shape(combined_neg)

    votes, descriptors, keypoints = zip(*combined_neg)
    bodypart_trained_data_neg.votes = np.array(votes)
    bodypart_trained_data_neg.descriptors = np.array(descriptors)
    bodypart_trained_data_neg.keypoints = np.array(keypoints)
    bodypart_trained_data_neg.bodypart = bodypart_neg
...
if __name__ == '__main__':
    parser = OptionParser()
    parser.add_option("", "--positive-training-list", dest="train_positive_files_list", help="File to save the information about the positive training data")
    parser.add_option("", "--negative-training-list", dest="train_negative_files_list", help="File to save the information about the negative training data")
