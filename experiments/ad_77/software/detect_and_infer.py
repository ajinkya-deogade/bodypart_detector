    ...
    for test_annotation_file in test_annotation_list:
        ...
        with open(test_annotation_file) as fin_annotation:
            ...
            for j in range(0, len(test_annotations)):
                ...
                print "len(buffer_detection['MouthHook']):", len(buffer_detection['MouthHook'])
                ...
                if (display_level >= 3):
                    ...
                    folderName = '../expts/annotatedFrames_Inferred/'
                    ...
                    folderName = '../expts/annotatedFrames_Estimated/'
                    ...
                    folderName = '../expts/annotatedFrames_EstimatedWithInferred/'
                    if not os.path.exists(folderName):
                        os.makedirs(folderName)
                    cv2.imwrite(os.path.join(folderName, os.path.splitext(os.path.basename(frame_file))[0]+'.png'), est_inf_Frame)

                    # est_inf_gt_Frame = np.concatenate((estimatedFrame, np.multiply(np.ones((1920, 200, 3), dtype=np.uint8), 255), inferredFrame, np.multiply(np.ones((1920, 200, 3), gtFrame))), axis=1)
                    # folderName = '../expts/annotatedFrames_EstimatedWithInferredAndGT/'
                    # if not os.path.exists(folderName):
                    #     os.makedirs(folderName)
                ...
                detections_error[frame_index] = computeErrors(bodypart_coords_est, bodypart_coords_gt)
                inferences_error[frame_index] = computeErrors(bodypart_coords_inf, bodypart_coords_gt)
                # det_and_inf_error[frame_index] = computeErrors(bodypart_coords_inf, bodypart_coords_est)
