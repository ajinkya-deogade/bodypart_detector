...
surf = cv2.xfeatures2d.SURF_create(250, nOctaves=2, nOctaveLayers=3, extended=0)
...
def plotErrors(datErr, datString):
    ...
    # pylab.show()
    pylab.savefig('../expts/figures/swarmPlot_' + datString + '_optimisation.png')
    pylab.clf()
...
def detect(detect_bodypart, project_dir, test_annotation_list, vote_sigma, vote_patch_size, vote_threshold, train_data_p):
    ...
    inferences_error_allTimes = {}
    for bp in infer_bodypart:
        inferences_error_allTimes[bp] = []

    for time_lag in range(3, 8):
            ...
            with open(test_annotation_file) as fin_annotation:
                ...
                for j in range(0, len(test_annotations)):
                    ...
                    if len(buffer_detection['MouthHook']) >= time_lag:
                        ...
                        for ibp in infer_bodypart:
                            if ibp in bodypart_coords_est:
                                bodypart_coords_inf[ibp] = {"x": bodypart_coords_est[ibp]["x"], "y": bodypart_coords_est[ibp]["y"]}
        ...
        for bp in infer_bodypart:
            inferences_error_allTimes[bp].append(np.median(inferences_error_all[bp]))

    for bp in inferences_error_allTimes:
        print inferences_error_allTimes[bp]
        print range(3, 8)
        plt.plot(np.squeeze(range(3, 8)), np.squeeze(inferences_error_allTimes[bp]), 'o-', linewidth=2)
        plt.ylabel('Median Distance from Ground-truth (pixels)', fontsize=14)
        plt.xlabel('Time lag (frames)', fontsize=14)
        plt.show()
        pylab.savefig('../expts/figures/linePlot_' + str(bp) + '_optimisation.png')
