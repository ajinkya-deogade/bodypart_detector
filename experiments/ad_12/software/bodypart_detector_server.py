    ...
    def handle(self):
        ...
        for i in range(0, packet["list"]["length"]):
            ...
            if(desc_frame is None):
                ...
                self.request.sendall(str(0).zfill(5))
                self.request.sendall(str(0).zfill(5))
                print 'NoneType'
            ...
            if ( vote_max > 0 and vote_max > frame_vote_max ):
                ...
                self.request.sendall(str(bodypart_coords_est["x"]).zfill(5))
                self.request.sendall(str(bodypart_coords_est["y"]).zfill(5))
            else:
        ...
        ack_message += '" , "detections" : [ { "frame_index" : ' + str(packet["list"]["frame_index"]) + ' , "test_bodypart" : "' + test_bodypart + '" , "coord_x" : ' + str(bodypart_coords_est["x"]) + ' , "coord_y" : ' + str(bodypart_coords_est["y"]) + ' } ] }'
